...
export default function Home() {
  ...
  const handleAdd = async (title: string, desc: string, author: string) => {
    // Optimistic UI - pseudo ID until server syncs
    const tempTask: Task = {
      id: Date.now(),
      title,
      description: desc,
    ...
    try {
      ...
      // the websocket message will trigger re-fetch and replace optimistic task with real task
    } catch (e) {
      console.error(e);
  ...
  return (
    <main className="min-h-screen relative font-sans text-gray-900 overflow-hidden">
      ...
      <div className="fixed inset-0 z-[-1] bg-gradient-to-br from-indigo-50 via-blue-50/50 to-cyan-50">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[30rem] h-[30rem] bg-cyan-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-6000"></div>
